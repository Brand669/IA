...
# Configuración PyTorch - Red neuronal simple
class RedNeuronal(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(16, 4)  # 4 posibles acciones: 0 = nada, 1 = saltar, 2 = izquierda, 3 = derecha
...
modelo_rn = RedNeuronal()
criterio = nn.CrossEntropyLoss()
optimizador = optim.Adam(modelo_rn.parameters(), lr=0.001)

datos_modelo = []

# Función para entrenar la red neuronal
def entrenar_modelo():
    global modelo_rn, datos_modelo
    if len(datos_modelo) < 10:
        print("No hay datos suficientes para entrenar la red neuronal.")
        return
    modelo_rn.train()
    X = np.array([list(caracteristicas) for caracteristicas, accion in datos_modelo], dtype=np.float32)
    y = np.array([accion for _, accion in datos_modelo], dtype=np.int64)
    X_tensor = torch.from_numpy(X)
    y_tensor = torch.from_numpy(y)
    
    for epoch in range(50):  # 50 épocas, ajustable
        optimizador.zero_grad()
        salida = modelo_rn(X_tensor)
        perdida = criterio(salida, y_tensor)
        perdida.backward()
        optimizador.step()
    print("Red neuronal entrenada con", len(datos_modelo), "datos.")

# Función para predecir acción con la red neuronal
def predecir_accion():
...
# Dimensiones y configuración Pygame (igual que tu código)
tamano = (800, 400)
w, h = tamano
pantalla = pygame.display.set_mode(tamano)
pygame.display.set_caption("Juego: Red Neuronal - Movimiento del Jugador")
...
def mostrar_menu():
    ...
    pantalla.fill(NEGRO)
    texto = fuente.render("Presiona 'R' para Auto (Red Neuronal), 'M' para Manual, o 'Q' para Salir", True, BLANCO)
