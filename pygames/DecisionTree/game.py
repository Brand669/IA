...
pygame.display.set_caption("Mono con Árbol de Decisión")
...
def predecir_accion():
    # Obtener datos actuales para predecir: velocidad bala, distancia horiz y vert
    dist_hor = abs(jugador.x - bala.x)
    dist_vert = abs(jugador.x - bala_vertical.x) if bala_vertical.y < h else 999
    entrada = np.array([[vel_bala, dist_hor, dist_vert]])
    try:
...
def guardar_datos(accion):
    dist_hor = abs(jugador.x - bala.x)
    dist_vert = abs(jugador.x - bala_vertical.x) if bala_vertical.y < h else 999
    datos_modelo.append(((vel_bala, dist_hor, dist_vert), accion))
