...
pygame.display.set_caption("Mono con Red Neuronal")
...
datos_modelo = []
modelo = MLPClassifier(hidden_layer_sizes=(20,), max_iter=1000)
...
def reiniciar_juego():
    ...
    jugador.x, jugador.y = POS_INICIAL_X, h - 100
...
def entrenar_modelo():
    ...
    if len(datos_modelo) > 10:  # mínimo 10 datos para entrenar
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_a:
                    modo_auto = True
                    entrenar_modelo()  # Entrena justo antes de modo auto
                    return
...
def guardar_datos(accion):
    dist_hor = abs(jugador.x - bala.x)
    dist_vert = abs(jugador.x - bala_vertical.x) if bala_vertical.y < h else 999
    # Agrego también posición para mejor entrenamiento
            ...
            if not pausa:
                if modo_auto:
                    ...
                    if accion == 1 and en_suelo:
                        ...
                        # Regresa suavemente a la posición inicial si no hay peligro
                        if jugador.x < POS_INICIAL_X:
                            jugador.x += 2
                        elif jugador.x > POS_INICIAL_X:
                            jugador.x -= 2
                else:
